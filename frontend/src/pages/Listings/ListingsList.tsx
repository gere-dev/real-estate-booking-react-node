...
export const ListingsList: React.FC<Props> = ({ property }) => {
  return (
    <li className='flex gap-4 shadow-lg shadow-zinc-200 p-3 rounded' key={property.property_id}>
      <figure className=''>
        <Link to={`/property/${property.property_id}`}>
          <img className='rounded-lg w-40 h-40 object-cover aspect-square' src={`${apiUrl}/uploads/${property.images[0]}`} alt='' />
        </Link>
      </figure>
